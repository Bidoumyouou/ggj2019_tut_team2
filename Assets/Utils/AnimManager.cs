	...
	protected override void UpdateAnimValue()
	{
		...
		if( timer_ >= updateTime_ || animValue_ <= 0.0f )
		{
			...
			if( shakeCount_ == 1 )
			{
				...
				{
					case ParamType.Position:
					transform_.localPosition = (Vector3)initialValue_ + initialVector_ * (float)Target * animValue_;
					break;
					case ParamType.PositionX:
					transform_.localPosition = new Vector3((float)initialValue_ + initialVector_.x * (float)Target * animValue_, transform_.localPosition.y, transform_.localPosition.z);
					break;
					case ParamType.PositionY:
					transform_.localPosition = new Vector3(transform_.localPosition.x, (float)initialValue_ + initialVector_.y * (float)Target * animValue_, transform_.localPosition.z);
					break;
					case ParamType.PositionZ:
					transform_.localPosition = new Vector3(transform_.localPosition.x, transform_.localPosition.y, (float)initialValue_ + initialVector_.z * (float)Target * animValue_);
					break;
				}
				return;
			}
